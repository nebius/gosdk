...
func (m *lifetimeMetricsRecorder) TokenAcquire(_ context.Context, provider string, result string, _ time.Duration, _ int) {
	m.acquireCount++
...
func (testTokenExchangeClient) Exchange(context.Context, *iampb.ExchangeTokenRequest, ...grpc.CallOption) (*iampb.CreateTokenResponse, error) {
...
func TestInstrumentedBearerTokenerPropagatesMetricsToWrappedTokener(t *testing.T) {
	...
	if _, err := wrapped.BearerToken(context.Background()); err != nil {
		t.Fatalf("bearer token: %v", err)
	}
	if metrics.cacheMissCount != 1 {
		t.Fatalf("cache miss count = %d, want 1", metrics.cacheMissCount)
