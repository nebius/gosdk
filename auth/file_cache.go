...
func (f *AsynchronouslyRenewableFileCachedTokener) requestToken(
	...
	res, err, _ := f.group.Do("", func() (any, error) {
		...
		if err != nil {
			f.recordRequestError(ctx, background, start)
			return nil, fmt.Errorf("get from wrapped tokener: %w", err)
		}
		f.recordAcquireResult(background, true)

		refreshAfter := f.refreshDelay(ctx, token, now)
		if storeErr := f.storeToken(ctx, token, background, start); storeErr != nil {
			return BearerToken{}, storeErr
		}
		...
		f.recordRequestSuccess(ctx, background, start)
		return token, nil
	})
	if err != nil {
		return BearerToken{}, err
...
func (f *AsynchronouslyRenewableFileCachedTokener) recordRequestError(
	ctx context.Context,
	background bool,
	start time.Time,
) {
	...
	if background {
		f.metrics.tokenRefresh(ctx, f, metricResultError, time.Since(start))
	}
}

func (f *AsynchronouslyRenewableFileCachedTokener) recordRequestSuccess(
	ctx context.Context,
	background bool,
	start time.Time,
) {
	if background {
		f.metrics.tokenRefresh(ctx, f, metricResultSuccess, time.Since(start))
	}
}

func (f *AsynchronouslyRenewableFileCachedTokener) refreshDelay(
	ctx context.Context,
	token BearerToken,
	now time.Time,
) time.Duration {
...
func (f *AsynchronouslyRenewableFileCachedTokener) storeToken(
	ctx context.Context,
	token BearerToken,
	background bool,
	start time.Time,
